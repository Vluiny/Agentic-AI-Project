...
# Menggunakan model cerdas untuk analisis paralel, dan model besar untuk sintesis
...
def analytical_mind_node(state: DecisionState):
    ...
    # Gunakan SystemMessage untuk instruksi role, bukan HumanMessage
    inputs = [SystemMessage(content=system_prompt)] + state["messages"]
    response = llm_fast.invoke(inputs)
    return {"messages": [HumanMessage(content=f"--- ANALISIS LOGIK & RISIKO ---\n{response.content}", name="Analytical_Mind")]}
...
def humanist_node(state: DecisionState):
    ...
    inputs = [SystemMessage(content=system_prompt)] + state["messages"]
    response = llm_fast.invoke(inputs)
    return {"messages": [HumanMessage(content=f"--- ANALISIS NILAI & MANUSIA ---\n{response.content}", name="The_Humanist")]}
...
def resource_investor_node(state: DecisionState):
    ...
    inputs = [SystemMessage(content=system_prompt)] + state["messages"]
    response = llm_fast.invoke(inputs)
    return {"messages": [HumanMessage(content=f"--- ANALISIS EKONOMI & SUMBER DAYA ---\n{response.content}", name="Resource_Investor")]}
...
def strategist_node(state: DecisionState):
    ...
    inputs = [SystemMessage(content=system_prompt)] + state["messages"]
    response = llm_fast.invoke(inputs)
    return {"messages": [HumanMessage(content=f"--- ANALISIS WAKTU & STRATEGI ---\n{response.content}", name="The_Strategist")]}


def social_lens_node(state: DecisionState):
    print("🌍 [Node 5] Agen Perspektif Sosial & Aturan Atasan sedang menganalisis...")
    system_prompt = (
    ...
    inputs = [SystemMessage(content=system_prompt)] + state["messages"]
    response = llm_fast.invoke(inputs)
    return {"messages": [HumanMessage(content=f"--- ANALISIS SOSIAL & REGULASI ---\n{response.content}", name="Social_Outside_Lens")]}
...
# --- Sisa inisialisasi alur LangGraph tetap sama ---
...
if __name__ == "__main__":
    user_dilemma = "besok sekolah atau tidak, teman teman tidak sekolah dan memang tidak ada kegiatan, tapi tidak ada pemberitahuan libur"
